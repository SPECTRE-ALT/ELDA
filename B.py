...
import json
import os        
import cv2                 # <-- ADDED for screen reading
import pytesseract         # <-- ADDED for screen reading
...
# --- MODIFIED: safe_type_string now handles 'Enter' ---
...
# 6. Main Chat Loop (Unchanged, but now calls new functions)
def main_chat_loop():
...
ACTIONS:
1. "type_string": The user wants to type something.
2. "open_application": The user wants to open a specific application.
3. "click_text": The user wants to find and click text on the screen.
4. "move_center": The user wants to move the mouse to the center.
5. "click_mouse": The user wants to click the mouse (at its current position).
6. "chat": The user is just chatting or asking a question.

JSON FORMAT:
- Type string:
{"action": "type_string", "text": "text to type", "press_enter": false, "confirmation": "Okay, typing that."}
(Note: "press_enter" is optional, default is false.)
...
(You MUST correct any spelling mistakes from the user's speech. If they say "dwonlaods", you must use "Downloads".)
...
---
**IMPORTANT CONTEXT RULE (Stateful Actions):**
If the user just opened an application (like "File Explorer") and their next command is related to that app (like "open downloads"), you MUST use the `type_string` action. Do NOT use `open_application` again.
...
Example 2:
User: "Launch Notepad"
You: {"action": "open_application", "app_name": "notepad", "confirmation": "Opening notepad."}
User: "type 'hello there'"
You: {"action": "type_string", "text": "hello there", "press_enter": false, "confirmation": "Typing 'hello there'."}

Example 3:
User: "Click on the 'File' menu"
You: {"action": "click_text", "text": "File", "confirmation": "Clicking on 'File'."}
"""
